...
class ForwardOutput(TypedDict):
    obs_rollout: Float[torch.Tensor, "batch sequence_length observable_dim"]
    obs_end_to_end: Float[torch.Tensor, "batch sequence_length observable_dim"]
    latent_rollout: Float[torch.Tensor, "batch sequence_length latent_dim"]
    latent_end_to_end: Float[torch.Tensor, "batch sequence_length latent_dim"]
...
class BaseModel(nn.Module):
    ...
    def extract_latent(
        self, x: Float[torch.Tensor, "batch observable_dim segment_length"]
    ) -> Float[torch.Tensor, "batch sequence_length latent_dim"]:
        """
    ...
    def rollout(
        self, x: Float[torch.Tensor, "batch context observable_dim"], output_length: int
    ) -> RolloutOutput:
        """

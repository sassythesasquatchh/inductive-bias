...
class BaseDecoder(nn.Module):
    ...
    ) -> Float[torch.Tensor, "batch observable_dim"]:
        """
...
class InformedDecoder(BaseDecoder):
    ...
    def forward(
        self, z: Float[torch.Tensor, "batch 3"]
    ) -> Float[torch.Tensor, "batch observable_dim"]:
        ...
        vel = z[:, 2:3] * self.sampling_positions
...
class UnstructuredDecoder(BaseDecoder):
    def __init__(self, latent_dim, observable_dim, hidden_dim, context, **kwargs):
        super().__init__(**kwargs)
        self.net = MLP(latent_dim, hidden_dim, observable_dim)

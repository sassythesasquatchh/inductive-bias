...
class HybridDynamics(BaseDynamics):
    def __init__(self, hidden_dim, context, config: Config, **kwargs):
        ...
        self.correction = MLP(3 * context, hidden_dim, 3 * context, hidden_layers=2)
...
class UnstructuredDynamics(BaseDynamics):
    def __init__(self, latent_dim, hidden_dim, context, **kwargs):
        ...
        self.context = context
        self.step = MLP(latent_dim * context, hidden_dim, latent_dim * context)

    def forward(
        self, z: Float[torch.Tensor, "batch context latent_dim"]
    ) -> Float[torch.Tensor, "batch context latent_dim"]:
        B, C, L = z.size()
        return self.step(z.reshape(B, C * L)).view(B, C, L)

...
def default_map_state_to_observation(state: np.ndarray) -> np.ndarray:
    ...
    # Note that the pendulum environment considers the pendulum to be upright when theta = 0,
    # however the encoders that we train consider the pendulum to be upright when theta = pi.
...
class PendulumEnv(gym.Env):
    ...
    def _dynamics(self, theta: float, torque: float) -> float:
        ...
        # angular acceleration: (torque - m*g*l*sin(theta)) / (m*l*l)
        return torque / (self.m * self.l**2) - (self.g / self.l) * np.sin(theta)

    def _rk4_step(self, state: np.ndarray, torque: float, dt: float) -> np.ndarray:
        """
        Performs one step using the 4th-order Runge-Kutta method (RK4).
        This is more accurate than the symplectic method but doesn't preserve
        the symplectic structure of Hamiltonian systems.

        System equations:
        theta_dot = thetadot
        thetadot_dot = -g/l * sin(theta) + torque / (m*l^2)

        RK4 steps:
        1. k1 = f(t_n, y_n)
        2. k2 = f(t_n + dt/2, y_n + dt/2 * k1)
        3. k3 = f(t_n + dt/2, y_n + dt/2 * k2)
        4. k4 = f(t_n + dt, y_n + dt * k3)
        5. y_{n+1} = y_n + dt/6 * (k1 + 2*k2 + 2*k3 + k4)
        """
        ...
        def derivatives(state):
            ...
            dtheta = thetadot
            dthetadot = self._dynamics(theta, torque)
        ...
        # Normalize angle and clip speed
        new_state[0] = self._angle_normalize(new_state[0])
        new_state[1] = np.clip(new_state[1], -self.max_speed, self.max_speed)

        return new_state.astype(np.float32)

    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, dict]:
        """
        Apply action, update state using solver, return observation.
        """
        ...
        self.state = self._rk4_step(self.state, torque, dt)
